...
app.get('/', (req, res) => {
	...
	if (order === 'asc') {
		queryString = 'select * from videos order by rating';
	} else if (order === 'desc') {
		queryString = 'select * from videos order by rating desc';
	}
...
app.post('/', (req, res) => {
	...
	pool
		.query(
			`insert into videos (title, url, rating, time, date) values ($1, $2, $3, $4, $5) returning id`,
			[title, url, 0, time, date]
		)
		.then((result) => {
			res.status(202).json(result.rows);
		});
	// .then(() => {
	// 	pool.query(`select * from videos`).then((responseUpdated) => {
	// 		res.status(202).json(responseUpdated.rows);
	// 	});
	// })
