...
const updateVideoData = (newVideoData) => {
  fetch (`https://server-7g43.onrender.com/videos/?order=${order}`, {
    method: "POST", headers: {"Content-Type": "application/json",}, body: JSON.stringify(newVideoData),
  })
  .then((response)=> response.json())
...
const deleteVideo = (videoId)=> {
  fetch(`https://server-7g43.onrender.com/videos/${videoId}/?order=${order}`, {
    method: "DELETE",
  })
  .then((response)=>response.json())
  .then((data)=> {setVideos(data);
...
const updateRating = (videoId, newRating) => {
  fetch (`https://server-7g43.onrender.com/videos/${videoId}`, {
    method: "PUT", headers: {"Content-Type": "application/json",}, body: JSON.stringify({id:videoId, rating:newRating}),
  })
  .then((response)=>response.json())
  .then((data)=> {setVideos(data);
  console.log(data);
